...
const { getProxyPathPrefix } = require('./utils')
...
const getPanel = () => {
    const panelUrl = `${config.basePath}/stubs/panel.js`.replace(/\/+/g, '/') // '//path' => '/path'
    return `
...
const setupPanel = (app, pathPrefix) => {
    app.use(pathPrefix + '/stubs', express.static(__dirname))
    app.use(stubsPanel)
}
...
const stubsMiddleware = (app, _config) => {
    ...
    const proxyPathPrefix = getProxyPathPrefix(app, config.basePath)

    Object.keys(config.proxy).map(key => {
        app.use(proxyPathPrefix + key, proxyMiddleware(key))
    })
    console.log('✅ stubs-middleware is running...')
    if (config.showPanel) {
        setupPanel(app, proxyPathPrefix)
    }
}
